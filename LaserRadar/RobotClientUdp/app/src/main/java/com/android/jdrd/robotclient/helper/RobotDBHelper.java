package com.android.jdrd.robotclient.helper;
...
/**
 * Created by xuzemin on 2017/6/11.
 */

public class RobotDBHelper extends DataBaseHelper {

    private static RobotDBHelper mTestDBHelper;

    private RobotDBHelper(Context context){
        super(context);
    }
    ...
    public static RobotDBHelper getInstance(Context context){
        if (mTestDBHelper==null){
            synchronized (DataBaseHelper.class){
                if (mTestDBHelper==null){
                    mTestDBHelper = new RobotDBHelper(context);
    ...
    protected int getMDbVersion(Context context) {
        return Integer.valueOf(context.getResources().getStringArray(R.array.DATABASE_INFO).length);
    }
