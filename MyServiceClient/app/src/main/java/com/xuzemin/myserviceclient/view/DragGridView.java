package com.xuzemin.myserviceclient.view;
...
//            //获取触摸点相对于屏幕的坐标
            ...
            int position = -1;
            if(downRawY > 792 && downRawY < 888 ){
                if(downRawX > 176 && downRawX < 334) {
                    position = 0;
                }else if(downRawX > 334 && downRawX < 474){
                    position = 1;
                }else if(downRawX > 474 && downRawX < 612){
                    position = 2;
                }else if(downRawX > 612 && downRawX < 744){
                    position = 3;
                }else if(downRawX > 744 && downRawX < 880){
                    position = 4;
                }
                if(position != -1){
                    //记录长按item位置
                    ...
                    //获取被长按item的drawing cache
                    getChildAt(position).destroyDrawingCache();
                    getChildAt(position).setDrawingCacheEnabled(true);
                    //通过被长按item，获取拖动item的bitmap
                    Bitmap dragBitmap = Bitmap.createBitmap(getChildAt(position).getDrawingCache());
