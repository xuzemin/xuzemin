 ...
 * 描述: 区域名称适配器
 */
public class ZB_SpinnerAdapter extends BaseAdapter {
    Context context;
    List<Map> list;
    boolean flag;

    public ZB_SpinnerAdapter(Context _context, List<Map> _list, boolean flag) {
        this.list = _list;
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (flag) {
            if (position == ZB_CommandActivity.goalNum) {
                viewHolder.imageview.setVisibility(View.VISIBLE);
            }
        } else {
            if (position == ZB_CommandActivity.directionNum) {
                viewHolder.imageview.setVisibility(View.VISIBLE);
            }
        }
