...
public class DemoActivity extends Activity implements PropertyChangeListener {
    ...
    private ServiceConnection serviceConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName className, IBinder service) {
            ...
            setLightbulb(switchPowerService.getManager().getImplementation()
                    .getStatus());
    ...
    protected void setLightbulb(final boolean on) {
        runOnUiThread(new Runnable() {
            public void run() {
                ...
                imageView.setImageResource(on ? R.drawable.lightbulb
    ...
    protected LocalService<SwitchPower> getSwitchPowerService() {
        ...
        if ((binaryLightDevice = upnpService.getRegistry().getLocalDevice(udn,
                true)) == null)
            return null;

        return (LocalService<SwitchPower>) binaryLightDevice
                .findService(new UDAServiceType("SwitchPower", 1));
    }
    ...
    protected LocalDevice createDevice() throws ValidationException,
        ...
        DeviceDetails details = new DeviceDetails("Friendly Binary Light",
                new ManufacturerDetails("ACME"), new ModelDetails(
                        "AndroidLight", "A demo light with on/off switch.",
                        "v1"));

        LocalService service = new AnnotationLocalServiceBinder()
                .read(SwitchPower.class);

        service.setManager(new DefaultServiceManager<SwitchPower>(service,
                SwitchPower.class));
