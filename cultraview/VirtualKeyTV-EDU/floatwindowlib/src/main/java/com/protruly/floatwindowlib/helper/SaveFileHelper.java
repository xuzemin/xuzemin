...
public class SaveFileHelper {
    ...
    public String createMeetingDirName(Context context) {
        ...
        String saveDirName = String.format(CommConst.MEETING_DIR_NAME, mmdd, "");

        ACache aCache = ACache.get(context);
        String nextDir = aCache.getAsString(CommConst.KEY_NEXT_MEETING_DIR);
        ...
        if (!TextUtils.isEmpty(nextDir)
            ...
            saveDirName += "01";
            aCache.put(CommConst.KEY_NEXT_MEETING_DIR, saveDirName);
        }
        return saveDirName;
    }
    ...
    public void saveNextMeetingDirName(Context context, String saveNameCurrent) {
        ACache aCache = ACache.get(context);
        String dirCache = aCache.getAsString(CommConst.KEY_NEXT_MEETING_DIR);
        LogUtils.d("createMeetingDirName dirCurrent->%s, saveNameCurrent->%s", dirCache, saveNameCurrent);
        ...
        // 递增保存为下一个会议记录的文件夹名称
        if (!TextUtils.isEmpty(dirCache) && dirCache.contains(saveDirName)){
            // 获得当前序列
            ...
            try{
                ...
                aCache.put(CommConst.KEY_NEXT_MEETING_DIR, dirNext);
