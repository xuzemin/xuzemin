...
import com.yinghe.whiteboardlib.utils.RootCmd;
...
//        changeETHType();
        ...
        new Thread(()->{
            CmdUtils.ShellCommandListener listener = new CmdUtils.ShellCommandListener(){
                @Override
                public void onCommandFinished(CmdUtils.CommandResult result) {
                    Log.d(TAG, "result->" + result.toString());
                }
            };

            try {
                Log.d(TAG, "CmdUtils start");
                ...
                Log.d(TAG, "RootCmd isRoot:" + isRoot);


                if (isRoot){
                    for (int i=0; i< cmdStr.length;i++){
                        RootCmd.execRootCmd(cmdStr[i]);
                    }
                } else {

                }

                CmdUtils.execCommand(false, listener, "ls");
                CmdUtils.execCommand(true, listener, cmdStr);
            } catch (Exception e){
                e.printStackTrace();
            }
        }).start();
