...
//        int styleIndex = sharedPreferences.getInt(CommonConsts.STYLE_INDEX, 0);

        int styleIndex = Settings.System.getInt(mActivity.getContentResolver(), CommonConsts.STYLE_INDEX, 0);
...
//                outRect.top = space;
    ...
    public void onClick(View view) {
        ...
        if (id == R.id.back_btn || id == R.id.btn_cancel) {
            ...
            // 判断styleIndex是否改变
            int index = themeBgAdapter.getLastPosition();
            int styleIndex = Settings.System.getInt(mActivity.getContentResolver(), CommonConsts.STYLE_INDEX, 0);
            if (index == styleIndex) {
                mActivity.finish();
                return;
...
//            //步骤3：将获取过来的值放入文件
//            editor.putInt(CommonConsts.STYLE_INDEX, themeBgAdapter.getLastPosition());
//            //步骤4：提交
//            editor.commit();

            Settings.System.putInt(mActivity.getContentResolver(), CommonConsts.STYLE_INDEX, index);
