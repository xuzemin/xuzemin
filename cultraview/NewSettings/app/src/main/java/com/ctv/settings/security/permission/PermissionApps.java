...
public class PermissionApps {
    ...
    public int getGrantedCount(ArraySet<String> launcherPkgs) {
        ...
        for (PermissionApp app : mPermApps) {
            if (!PermissionUtils.shouldShowPermission(app)) {
                continue;
            }
            if (PermissionUtils.isSystem(app, launcherPkgs)) {
    ...
    private void loadGroupInfo() {
        ...
        mIcon = PermissionUtils.applyTint(mContext, mIcon, android.R.attr.colorControlNormal);
    }
    ...
    public static class PmCache {
        ...
        public synchronized List<PackageInfo> getPackages(int userId) {
            ...
            if (ret == null) {
                ret = mPm.getInstalledPackages(PackageManager.GET_PERMISSIONS);
