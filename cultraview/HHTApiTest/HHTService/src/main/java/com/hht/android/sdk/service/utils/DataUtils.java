...
//        String uriString = "content://com.cultraview.ctvmenu/sourcename/query";
        ...
        if (cursor != null && cursor.moveToFirst()){ // 选择自定义名称
            do {
                ...
                Log.d(TAG, "querySourceName, sourceEditName:" + sourceEditName);
    ...
    public void querySourceInfos(Context context, Map<String, String> srcMap) {
        if (HHTConstant.DEBUG) Log.d(TAG, "querySourceInfos, start");
        ContentResolver contentResolver = context.getContentResolver();
//        String uriString = "content://com.cultraview.ctvmenu/sourcename/query";
        Uri uri = URI_QUERY; //Uri.parse(uriString);
        Cursor cursor = contentResolver.query(uri, null, null,
                null, null);
        if (cursor != null && cursor.moveToFirst()){ // 选择自定义名称
            if (HHTConstant.DEBUG) Log.d(TAG, "querySourceInfos, cursor is not null");
            do {
                ...
                if (!TextUtils.isEmpty(sourceEditName)
                    ...
                    Log.d(TAG, "querySourceInfos, sourceNameKey:" + sourceNameKey + ", sourceEditName:" + sourceEditName);
        ...
        if (HHTConstant.DEBUG) Log.d(TAG, "querySourceInfos, end");
    }
    ...
    public void updateSourceName(Context context, String sourceNameKey, String sourceEdit) {
        Log.d(TAG, "sourceNameKey->" + sourceNameKey + " sourceEdit->" + sourceEdit);
...
/*        if (bAutoSourceSwitch == 0 && bSourceDetectEnable == 0) {
            Log.d("chen", "getSignalSwitchMode:state:close");
            return 0;
        } else if (bAutoSourceSwitch == 1 && bSourceDetectEnable == 1) {
            Log.d("chen", "getSignalSwitchMode:state:open");
            return 1;
        }*/
