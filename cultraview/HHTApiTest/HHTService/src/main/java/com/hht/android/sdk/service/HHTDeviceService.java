...
public class HHTDeviceService extends IHHTDevice.Stub {
    ...
    public HHTDeviceService(Context context){
        ...
        databaseDesk = DatabaseDesk.getInstance(context.getApplicationContext());
        Log.i("gyx","HHTDeviceService init");
    }
...
//        }
    ...
    @Override
    public String readCmdStrToTVOS(int iCmd) throws RemoteException {
        HHTDeviceManager.EnumRWCmd[] values = HHTDeviceManager.EnumRWCmd.values();
        if (iCmd < 0 || iCmd >= values.length){
            return "";
        }
        String cmdStr = values[iCmd].toString();
        try {
//            TvManager.getInstance().readCmdStrToTVOS(cmdStr);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return "";
    ...
    public boolean setBrightnessValue(int iVal) throws RemoteException {
        TvPictureManager.getInstance().setVideoItem(TvPictureManager.PICTURE_BRIGHTNESS, iVal);
        return true;
    }
    ...
    public boolean writeCmdStrToTVOS(int iCmd, String strData) throws RemoteException {
        ...
        String cmdStr = values[iCmd].toString() + "#" + strData;
        try {
