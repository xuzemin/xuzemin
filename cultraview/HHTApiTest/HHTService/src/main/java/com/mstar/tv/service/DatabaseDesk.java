...
public class DatabaseDesk implements IDatabaseDesk {
    ...
    @Override
    public void updateNR(int inputSrcType, int pictureModeIdx, EnumNoiseReduction eNRIdx) {
        ContentValues vals = new ContentValues();
        vals.put("eNR", eNRIdx.getValue());
        try {
            getContentResolver().update(
                    Uri.parse("content://mstar.tv.usersetting/nrmode/nrmode/" + pictureModeIdx
                            + "/inputsrc/" + inputSrcType), vals, null, null);
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    @Override
    public EnumNrSetting queryNR(int inputSrcType, int pictureModeIdx) {
        Cursor cursor = getContentResolver().query(
                Uri.parse("content://mstar.tv.usersetting/nrmode/nrmode/" + pictureModeIdx
    ...
    @Override
    public EnumMpegNrSetting queryMpegNR(int inputSrcType, int pictureModeIdx) {
        Cursor cursor = getContentResolver().query(
                Uri.parse("content://mstar.tv.usersetting/nrmode/nrmode/" + pictureModeIdx
                        + "/inputsrc/" + inputSrcType), null, null, null, null);
    ...
    @Override
    public void updateMpegNR(int inputSrcType, int pictureModeIdx, EnumMpegNoiseReduction eMpNRIdx) {
        ContentValues vals = new ContentValues();
    ...
    public UserSetting queryUserSysSetting() {
        ...
        if (cursor.moveToFirst()) {
            ...
            try {
                mUserSetting.eSpdifMode = EnumSpdifType.values()[cursor
                        .getInt(cursor.getColumnIndex("enSPDIFMODE"))];
            } catch (Exception e) {
                e.printStackTrace();
                mUserSetting.eSpdifMode = EnumSpdifType.values()[0];
            }
        }
        cursor.close();
        return mUserSetting;
    ...
    public UserSoundSetting querySoundSetting() {
        ...
        if (cursor.moveToFirst()) {
            ...
            int vdIdx = cursor.getColumnIndex("bEnableVideoDescription");
            if (0 <= vdIdx) {
                mSoundSetting.isVDEnable = cursor.getInt(cursor.getColumnIndex("bEnableVideoDescription")) == 0 ? false
                    : true;
            } else {
                mSoundSetting.isVDEnable = false;
            }
            ...
            int idx = cursor.getColumnIndex("ADMixerBalance");
            if (0 <= idx) {
                mSoundSetting.adMixerBalance = (short)cursor.getInt(idx);
            } else {
                mSoundSetting.adMixerBalance = (short)0;
            }
    ...
    public void updateSoundSetting(UserSoundSetting model) {
        ...
        vals.put("bEnableVideoDescription", model.isVDEnable ? 1 : 0);
        vals.put("bEnableAD", model.isADEnable ? 1 : 0);
        vals.put("ADVolume", model.adVolume);
        vals.put("ADMixerBalance", model.adMixerBalance);
    ...
    public ExternSetting queryFactoryExtern() {
        ...
        if (cursor.moveToFirst()) {
            ...
            model.energyLogenable = cursor.getInt(cursor.getColumnIndex("energyLogenable")) != 0;
            model.energyModeEnable = cursor.getInt(cursor.getColumnIndex("energyModeEnable")) != 0;
            model.uiBrightness = cursor.getInt(cursor.getColumnIndex("uiBrightness"));
            model.uiContrast = cursor.getInt(cursor.getColumnIndex("uiContrast"));
            // CTV Patch end
            try {
                model.isNoSignalAutoShutdownEnable = cursor.getInt(
                        cursor.getColumnIndexOrThrow("NoSignalAutoShutdown")) != 0;
            } catch (IllegalArgumentException e) {
                e.printStackTrace();
    ...
    public PeqSetting queryPEQAdjusts() {
        ...
        while (cursor.moveToNext()) {
            ...
            int idx = cursor.getColumnIndex("Enable");
            if (0 <= idx) {
                model.peqParams[i].enable = cursor.getInt(idx);
            } else {
                model.peqParams[i].enable = 0;
            }
    ...
    public void updateSelfAdaptiveLevel(int value, int inputSrcType) {
        ...
        if (ret == -1) {
            Log.e(TAG, "updateSelfAdaptiveLevel fail");
        }
    ...
    @Override
    public int queryTtsSetting() {
        Cursor cursor = getContentResolver().query(
                Uri.parse("content://mstar.tv.usersetting/miscsetting/"), null, null, null, null);
        int rank = 0;
        if (cursor.moveToFirst()) {
            final int columnIdx = cursor.getColumnIndex("TTSSetting");
            if (-1 == columnIdx) {
                /* Index: -1 if the column doesn't exist */
                Log.e(TAG, "Fatal error! DB field TTSSetting doesn't exist!");
            } else {
                rank = cursor.getInt(columnIdx);
            }
        }
        cursor.close();
        return rank;
    ...
    @Override
    public void updateTtsSetting(int data) {
        ContentValues vals = new ContentValues();
        vals.put("TTSSetting", data);
        try {
            getContentResolver().update(Uri.parse("content://mstar.tv.usersetting/miscsetting"),
                    vals, null, null);
        } catch (SQLException e) {
    ...
    public List<RR5RatingPair> queryRR5RatingPair(int index, int count) {
        Log.d(TAG, "queryRR5RatingPair index = " + index + ", count = " + count);
        List<RR5RatingPair> list = new ArrayList<RR5RatingPair>();
        List<Integer> valueList = new ArrayList<Integer>();

        Cursor cursorV = getContentResolver().query(
                Uri.parse("content://mstar.tv.usersetting/rr5ratingpair/" + index), null, null, null, null);
        while (cursorV.moveToNext()) {
            for (int idx = 0; idx < 15; idx++) {
                final int v = cursorV.getInt(cursorV.getColumnIndex("stRR5RatingPair_id_" + idx));
                Log.d(TAG, "idx = " + idx + ", value = " + v);
                valueList.add(v);
            }
            break;
        }
        ...
        Cursor cursorA = getContentResolver().query(
                Uri.parse("content://mstar.tv.usersetting/abbratingtext/" + index), null, null, null, new String("_id asc"));
        int countA = 0;
        while (cursorA.moveToNext() && countA < count) {
            String stAbbRatingText = cursorA.getString(cursorA.getColumnIndex("stAbbRatingText"));
            final int id = cursorA.getInt(cursorA.getColumnIndex("_id"));
            RR5RatingPair pair = new RR5RatingPair();
            pair.regin5Dimension_index = countA;
            pair.abbRatingText = stAbbRatingText;
            pair.rR5RatingPair_id = id;
            if (valueList.size() > countA) {
                pair.value = valueList.get(countA);
            } else {
                pair.value = 0;
            }
            list.add(pair);
    ...
    @Override
    public String queryRRT5Region5Name() {
        Cursor cursor = getContentResolver().query(
                Uri.parse("content://mstar.tv.usersetting/vchipratinginfo/"), null, null, null,
                null);
        String region5Name = "";
        if (cursor.moveToFirst()) {
            region5Name = cursor.getString(cursor.getColumnIndex("u8Region5Name"));
        }
        cursor.close();
        return region5Name;
    }
