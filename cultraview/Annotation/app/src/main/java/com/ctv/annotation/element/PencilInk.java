...
public class PencilInk extends Material {
    ...
    public PencilInk(PanelManager manager, int color, float thickness,int mType)
    {
    ...
    protected void initPaint()
    {
        ...
        float pWidth = mInk.mThickness/2;
        if(PenType == 1) {
            Path p = new Path();
    ...
    public ArrayList<PencilInk> splitPencil(Polygon polygon)
    {
        ...
        if (Rect.intersects(pathRect, polygonRect))
        {
            ...
            for (int i = 0; i < mInk.mPointCount; i++)
            {
                ...
                if (point != null)
                {
                    int rectIndex = polygon.containsInRect((int) point.mX, (int) point.mY);
                    Point p;
                    if (rectIndex != -1)
                    {
                        p = point;
    ...
    private Point computeInterSectPoint(Polygon polygon, Point inside, Point outside, int rectIndex)
    {
        ...
        float deltaX = (inside.mX - outside.mX) / 20.0f;
        float deltaY = (inside.mY - outside.mY) / 20.0f;
        int count = 0;
        do
        {
            count++;
    ...
    {
        return new PencilInk(mManager, mInk.mInkColor, mInk.mThickness,PenType);
    }
    ...
    @Override public void transform(Matrix matrix)
    {
        endTransform(matrix);
...
//        mPath.rewind(); // 添加 0813 1021
    ...
    private void insertFalsePoints(float pX, float pY)
    {
        float dist = (float) Math.sqrt((double) (((mPreviousX - pX) * (mPreviousX - pX)) + ((mPreviousY - pY) * (mPreviousY - pY))));
        if (dist > 20.0f)
        {
            int pointCount = ((int) (dist / 20.0f));
            float deltaX = (pX - mPreviousX) / ((float) pointCount);
            float deltaY = (pY - mPreviousY) / ((float) pointCount);
            float tempX = mPreviousX;
            float tempY = mPreviousY;
            for (int i = 0; i < pointCount; i++)
            {
                tempX += deltaX;
                tempY += deltaY;
                savePointsToArray(tempX, tempY, true);
            }
        }
    }

    protected void savePointsToArray(float pX, float pY, boolean pIsFalsePoint)
    {
        enlargePointArrayIfNecessary();
        Point point = new Point(pX, pY);
        point.mIsFalsePoint = pIsFalsePoint;
        ...
        if (this.mIsOpenGLEnabled && !pIsFalsePoint)
        {
            enlargeGlPointIfNecessary();
            this.mGLPoints[this.mGLPointCount * 2] = pX;
            this.mGLPoints[(this.mGLPointCount * 2) + 1] = pY;
            this.mGLPointCount++;
        }
    }
    ...
    public void drawCurrentPath(Canvas pCanvas)
    {
        if (mCurrentPath != null)
        {
            pCanvas.drawPath(mCurrentPath, mPaint);
        }
    }
...
//        Log.d(TAG, "insertFalsePoints --- prePoint = " + prePoint.toString() + ", curPoint = " + curPoint.toString());
    ...
    public boolean readObject(InputStream in) throws IOException
    {
        ...
        if (read != 4)
        {
           // ToofifiLog.e(Constants.TAG, "Pen: base datas");
            return false;
        }
        setId(id);
        mPaint.setColor(mInk.mInkColor);
