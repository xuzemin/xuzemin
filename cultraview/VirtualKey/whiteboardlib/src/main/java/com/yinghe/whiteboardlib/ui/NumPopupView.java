...
public class NumPopupView {
    ...
    public NumPopupView(Context context, TextView textView) {
    ...
    private void initView(){
        // 菜单弹框布局
        View numPopupLayout = LayoutInflater.from(context).inflate(R.layout.pop_nums_layout, null);
        ...
        //菜单弹窗
        numPopupWindow = new PopupWindow(context);
        numPopupWindow.setContentView(numPopupLayout);//设置主体布局

        // 设置弹窗大小
        numPopupWindow.setWidth(ScreenUtils.dip2px(context, pupWindowsDPWidth));//宽度
        numPopupWindow.setHeight(ScreenUtils.dip2px(context, menuPupWindowsDPHeight));//高度
    ...
    public void showNumPopupWindow(View anchor) {
        numPopupWindow.showAsDropDown(anchor, ScreenUtils.dip2px(context, 0),
                ScreenUtils.dip2px(context, 200 + DrawConsts.POPUP_WIN_YOFF + 30));
    }
