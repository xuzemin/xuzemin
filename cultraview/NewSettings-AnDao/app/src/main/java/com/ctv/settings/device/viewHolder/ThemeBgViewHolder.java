...
public class ThemeBgViewHolder extends BaseViewHolder implements View.OnClickListener {
    ...
    public void initData(Activity activity) {
        ...
        String[] nameArray = mActivity.getResources().getStringArray(R.array.themeBgNameID);
...
//                outRect.top = space;
    ...
    public void onClick(View view) {
        ...
        if (id == R.id.btn_cancel) {
            ...
            // 判断选中的是否与上一次相同
            int currentIndex = themeBgAdapter.getLastPosition();
            SharedPreferences sharedPreferences = mActivity.getSharedPreferences("data", Context.MODE_PRIVATE);
            int lastBgIndex = sharedPreferences.getInt("bgIndex", 0);
            L.d(TAG, "onItemClick----->currentIndex:%s, lastBgIndex:%s",currentIndex, lastBgIndex);
            if (currentIndex == lastBgIndex) {
                mActivity.finish();
                return;
            }
            ...
            try {
                ...
                setWallResource(wallpaperManager, currentIndex);

                //步骤2： 实例化SharedPreferences.Editor对象
                SharedPreferences.Editor editor = sharedPreferences.edit();
                //步骤3：将获取过来的值放入文件
                editor.putInt("bgIndex", currentIndex);
                //步骤4：提交
                editor.commit();

                // 保存主题值
                MyApp app = (MyApp) mActivity.getApplication();
                app.themeResID = Tools.getThemeID(mActivity, currentIndex);

                ExitAppUtil.getInstance().clear();

                Intent i = new Intent("com.android.device.THEME_BG_ACTION");
                i.putExtra("data", currentIndex);
                mActivity.sendBroadcast(i);

                Intent intent = new Intent();
                String title = list.get(currentIndex).getTitle();
