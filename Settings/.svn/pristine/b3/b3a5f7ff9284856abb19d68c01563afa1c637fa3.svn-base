...
package com.ctv.settings.network.helper;
...
import com.ctv.settings.network.utils.WifiSecurity;
...
public final class WifiConfigHelper {
    ...
    public static boolean areSameNetwork(WifiManager wifiManager, ScanResult scanResult,
            WifiInfo wifiInfo) throws NoSuchMethodException, IllegalAccessException, InvocationTargetException {
        ...
        String wifiInfoSSID = NetUtils.removeDoubleQuotes(wifiInfo,wifiInfo.getSSID());
//        String wifiInfoSSID = WifiInfo.removeDoubleQuotes(wifiInfo.getSSID());
    ...
    public static WifiConfiguration getConfigurationForNetwork(Context context,
            ScanResult network) throws NoSuchMethodException, IllegalAccessException, InvocationTargetException {
    ...
    public static WifiConfiguration getConfiguration(Context context, String ssid,
            WifiSecurity security) throws NoSuchMethodException, IllegalAccessException, InvocationTargetException {
     ...
     */
//    public static void forgetConfiguration(Context context, WifiConfiguration config, ActionListener listener) throws ClassNotFoundException, NoSuchMethodException, IllegalAccessException, InvocationTargetException {
//        if (config == null) {
//            return;
//        }
//
...
////                wifiMan.forget(wc.networkId, listener);
//                NetUtils.forget(wifiMan,wc.networkId,listener);
...
//    }
    ...
    public static void forgetConfiguration(Context context, WifiConfiguration config) throws ClassNotFoundException, NoSuchMethodException, IllegalAccessException, InvocationTargetException {
        if (config == null) {
            return;
        }
        ...
        for (WifiConfiguration wc : configuredNetworks) {
            if (wc != null && wc.SSID != null && TextUtils.equals(wc.SSID, config.SSID)) {
//                wifiMan.forget(wc.networkId, listener);
                NetUtils.forgetNull(wifiMan,wc.networkId);
                if (DEBUG) Log.d(TAG, "forgot network config: " + wc.toString());
                break;
            }
        }
     ...
     */
    public static void forgetWifiNetwork(Context context) throws ClassNotFoundException, NoSuchMethodException, IllegalAccessException, InvocationTargetException {
...
//                String configuredSsid = WifiInfo.removeDoubleQuotes(configuredNetwork.SSID);
                String configuredSsid = NetUtils.removeDoubleQuotes(wifiInfo,configuredNetwork.SSID);
