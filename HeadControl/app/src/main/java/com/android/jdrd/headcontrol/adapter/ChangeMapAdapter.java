...
//        final float b = arrayPlanLists.get(key).get("point_ys").get(position) / 90 + 1;
        ...
        String str = String.valueOf(arraylist.get(key).get("point_xs").get(position) / 90);
        layoutTag.change_number_xs.setText(str);
        str = String.valueOf(10 - (arraylist.get(key).get("point_ys").get(position) / 90));
        layoutTag.change_number_ys.setText(str);
...
//                handler.sendEmptyMessage(5);
    ...
    public void update(int index,ListView listview){
        ...
        String str = String.valueOf(arraylist.get(key).get("point_xs").get(index - visiblePosition) / 90);
        holder.change_number_xs.setText(str);
        str = String.valueOf((10 - (arraylist.get(key).get("point_ys").get(index - visiblePosition) / 90)));
        holder.change_number_ys.setText(str);
    }
}
