 ...
 * </p>
 */
public class StaggeredGridViews extends ViewGroup {
	private static final String TAG = "StaggeredGridViews";
	...
	private int mColCountSetting = -1;
	...
	public StaggeredGridViews(Context context) {
		this(context, null);
	}

	public StaggeredGridViews(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
		if (attrs != null) {
			TypedArray a = context.getTheme().obtainStyledAttributes(attrs,
					R.styleable.StaggeredGridViewAttr, 0, 0);

			mMinColWidth = (int) a.getDimension(
					R.styleable.StaggeredGridViewAttr_minColWidth, 0);
			mColCountSetting = a.getInteger(
					R.styleable.StaggeredGridViewAttr_columnsSetting, 3);
			mColCount = a.getInteger(
					R.styleable.StaggeredGridViewAttr_numColumns, 3);
			mItemMargin = a.getInteger(
					R.styleable.StaggeredGridViewAttr_numColumns, 0);
			mDrawSelectorOnTop = a.getBoolean(
					R.styleable.StaggeredGridViewAttr_drawSelectorOnTop, false);
		} else {
			mColCountSetting = mColCount = 2;
			mDrawSelectorOnTop = false;
		}
	}

	public StaggeredGridViews(Context context, AttributeSet attrs, int defStyle) {
	...
	private void displayMapping() {
		...
		for (ArrayList<Integer> map : this.mColMappings) {
			...
			LogUtil.w("DISPLAY", sb.toString());
			sb = new StringBuilder();
			col++;
		}
		LogUtil.w("DISPLAY", "MAP END ****************");
	}
	...
	public void setAdapter(ListAdapter adapter) {
		...
		// the state here.
		clearAllState();
		mAdapter = new HeaderFooterListAdapter(mHeaderView, mFooterView,
				adapter);
		mDataChanged = true;
			...
			if (this.mColWidth > 0)
				for (int i = 0; i < mColCount; i++) {
					if (getChildAt(i) != null) {
						...
						final int left = child.getLeft();
						int col = 0;
						LogUtil.w("mColWidth", mColWidth + " " + left);
	...
	public static class LayoutParams extends ViewGroup.LayoutParams {
		...
		public LayoutParams(Context c, AttributeSet attrs) {
			...
			if (this.width != MATCH_PARENT) {
				LogUtil.w(TAG, "Inflation setting LayoutParams width to "
		...
		public LayoutParams(ViewGroup.LayoutParams other) {
			...
			if (this.width != MATCH_PARENT) {
				LogUtil.w(TAG, "Constructing LayoutParams with width " + this.width
				...
				if (lastCount != mItemCount) {
					...
					if (childCount > 0) {
						...
						for (int i = 0; i < childCount; i++) {
							...
							final int top = child.getTop() - mItemMargin;
							final int bottom = child.getBottom();
							final LayoutRecord rec = mLayoutRecords
									.get(mFirstPosition + i);

							final int colEnd = Math.min(mColCount, lp.column
									+ lp.span);
							for (int col = lp.column; col < colEnd; col++) {
								final int colTop = top;
								final int colBottom = bottom;
	...
	public interface OnItemClickListener {
		 ...
		 */
		void onItemClick(StaggeredGridViews parent, View view, int position,
				long id);
	}
	...
	public interface OnItemLongClickListener {
		 ...
		 */
		boolean onItemLongClick(StaggeredGridViews parent, View view,
	...
	public void setHeaderView(View v) {
		...
		StaggeredGridViews.LayoutParams lp = new StaggeredGridViews.LayoutParams(
