...
public class ImageLoaderUtils extends ImageLoader {
	 ...
	 *            加载用户头像，
	 */
	public void displayUserHeadPicWithOutCache(String uri, ImageView imageView) {
		if (instance == null)
			instance = getInstance();
		try {
			instance.displayImage(
					...
					new DisplayImageOptions.Builder()
							.showImageOnLoading(R.drawable.default_head_pic)
							.showImageForEmptyUri(R.drawable.default_head_pic)
							.showImageOnFail(R.drawable.default_head_pic)
							.cacheInMemory(false)
							// default 不缓存至内存
							.cacheOnDisc(false)
	...
	public DisplayImageOptions getOptionsWithOutCache() {
		if (optionsWithOutCache == null) {
			optionsWithOutCache = new DisplayImageOptions.Builder()
					.showImageOnLoading(R.drawable.load_failure)
					.showImageForEmptyUri(R.drawable.load_failure)
