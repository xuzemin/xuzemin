...
public class SJX_CommandActivity extends Activity implements View.OnClickListener, CompoundButton.OnCheckedChangeListener {
    // 初始化数据库帮助类
    ...
    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
        switch (buttonView.getId()) {
            case R.id.cb_top:
                if (cb_top.isChecked()) {
                    up_obstacle = 0;
                    robotDBHelper.execSQL("update command set up_obstacle = '" + up_obstacle + "' where id= '" + command_id + "'");
                } else {
                    up_obstacle = 1;
                    robotDBHelper.execSQL("update command set up_obstacle = '" + up_obstacle + "' where id= '" + command_id + "'");
                }
            ...
            case R.id.cb_bottom:
                if (cb_bottom.isChecked()) {
                    up_obstacle = 0;
                    robotDBHelper.execSQL("update command set down_obstacle = '" + up_obstacle + "' where id= '" + command_id + "'");
                } else {
                    up_obstacle = 1;
                    robotDBHelper.execSQL("update command set down_obstacle = '" + up_obstacle + "' where id= '" + command_id + "'");
                }
            ...
            case R.id.cb_side:
                if (cb_side.isChecked()) {
                    up_obstacle = 0;
                    robotDBHelper.execSQL("update command set side_obstacle = '" + up_obstacle + "' where id= '" + command_id + "'");
                } else {
                    up_obstacle = 1;
                    robotDBHelper.execSQL("update command set side_obstacle = '" + up_obstacle + "' where id= '" + command_id + "'");
                }
