...
import com.android.jdrd.robot.util.PreferencesUtils;
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (AreaIsEdit) {
                    if (position == 0) {
                        ...
                        Intent intent = new Intent(SJX_MainActivity.this, SJX_AreaRedact.class);
...
//                        dialog(areaData_list.get(position).get("name").toString(), (int) areaData_list.get(position).get("id"));
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.card:
                // 跳转到系统卡页面CardConfig
                startActivity(new Intent(SJX_MainActivity.this, SJX_CardConfig.class));
                break;
            // 机器人列表整体
            ...
            case R.id.up:
                ...
                // 清除命令集合
                robotDialog(Protocol.getSendData(Protocol.CONTROL_CLEAR, Protocol.getCommandDataByte(Protocol.ROBOT_CONTROL_CLEAR)));
                break;
            // 后退命令
            case R.id.down:
                //robotDialog("*d+6+#");// 字符串
